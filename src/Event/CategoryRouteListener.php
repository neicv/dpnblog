...
class CategoryRouteListener implements EventSubscriberInterface
{
    ...
    {
        if ($route->getName() == '@dpnblog/category' && CategoryUrlResolver::getPermalink()) {
            App::routes()->alias(dirname($route->getPath()).'/'.ltrim(CategoryUrlResolver::getPermalink(), '/'), '@dpnblog/category', ['_resolver' => 'Pastheme\Blog\CategoryUrlResolver']);
        }
